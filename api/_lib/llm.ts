...
export async function generateEarthquakeReply(
  ...
  // 6) Forecast multi-hari (kalau belum ada API forecast, jawab mitigasi + rujukan)
...
function isHeavyRainSignal(alert: NowcastAlert): boolean {
  const t = `${alert.headline ?? ''} ${alert.event ?? ''} ${alert.description ?? ''}`.toLowerCase();
  return (
...
function immediateDangerReply(message: string, input: ChatContextInput): string {
  const loc = readString(input.userLocation?.label, '');
  const locText = loc ? ` (lokasi terdeteksi: ${loc})` : '';

  // Prioritize based on hints
  if (isFireQuery(message) || message.includes('asap') || message.includes('api')) {
    return [
      'Status: Tinggi',
  ...
  if (isFloodQuery(message) || message.includes('arus') || message.includes('hanyut')) {
    return [
      'Status: Tinggi',
  ...
  if (message.includes('pingsan') || message.includes('tidak sadar') || message.includes('sesak') || message.includes('perdarahan')) {
    return [
      'Status: Tinggi',
...
function forecastFallbackReply(message: string, input: ChatContextInput): string {
  const location =
    extractLocationFromMessage(message) ?? readString(input.userLocation?.label, '');

  const locText = location ? ` di ${location}` : '';

  // Fokus: user ingin prakiraan → jika belum ada API, berikan mitigasi + cara cek resmi
  if (isFloodQuery(message) || message.includes('hujan') || message.includes('cuaca')) {
    return [
      'Status: Sedang',
...
function fallbackReply(input: ChatContextInput): string {
  ...
  if (message.includes('cedera') || message.includes('luka') || message.includes('perdarahan')) {
    return [
      'Status: Tinggi',
...
function formatLatestEventReply(input: ChatContextInput): string | null {
  ...
  parts.push(`Ringkas: Gempa terbaru M${magnitudeText} kedalaman ${depthText} km di ${wilayah}.`);
  parts.push('Langkah cepat:');
  parts.push('1) Jika masih ada guncangan/aftershock: lakukan Drop–Cover–Hold.');
  parts.push('2) Jauhi bangunan retak, cek gas/listrik, siapkan evakuasi jika diperlukan.');
